...
func NixPrint(file *File) ([]byte, error) {
	p := newPrinter(file)
	return p.Print()
}

func NixPrintExpression(expression Expression) ([]byte, error) {
	dummyFile := &File{}
...
func (p *printer) printHeader() {
	...
	p.output = append(p.output, header...)
}
...
func (p *printer) printModule(module *Module) {
	...
	p.printToken("=", noPos)
	p.requestSpace()
	p.printToken(module.Type, noPos)
...
func (p *printer) printOperatorInternal(operator *Operator, allowIndent bool) {
	...
	// Need to eval to know the type of nix concatenation to use
	// blueprint only supports "+" operator currently
	e1 := operator.Args[0].Eval()
	switch e1.Type() {
	case StringType:
		p.printToken("+", operator.OperatorPos)
	case Int64Type:
		p.printToken("+", operator.OperatorPos)
	case ListType:
		p.printToken("++", operator.OperatorPos)
	default:
		panic(fmt.Errorf("Dont know how to handle '+' operator with %s", e1.Type()))
	}
